...
int main(int argc, char** argv)
{
    ...
    while(true)
    {
        ...
        c = getopt_long(argc, argv, "ihXc:l:t:b:L:C:f:F:Rr:AN:P:T:W:S:D:", long_options, &option_ind);
        if (c == -1)
            break;
        switch(c)
        {
            ...
            case 'h':
                ...
                printf("  --hexagon\t\t(короткий ключ -X)\n\t\tНе требует аргументов. Функция рисует шестиугольник: либо с помощью окружности, либо с помощью квадрата.\n");
                printf("\t\tДля окружности нужно задать координаты центра и ее радиус (опции --circle_centre (-C) b --radius (-l)\n");
                ...
                printf("  --radius\t\t(короткий ключ -l)\n\t\tТребует одного параметра: радиуc для окружности.\n\n");
                printf("  --top_coords\t\t(короткий ключ -t)\n\t\tТребует два аргумента: левую верхнюю координату по оси OX и по оси OY\n\n");
                printf("  --bottom_coords\t\t(короткий ключ -b)\n\t\tТребует два аргумента: правую нижнюю координату по оси OX и по оси OY\n\n");
                printf("  --line_thickness\t\t(короткий ключ -L)\n\t\tТребует один аргумент: толщину линии.\n\n");
                ...
                printf("\t\tМожно испольовать для задания цветов линии или для изначальных пикселей.\n\n");
                ...
                printf("  --region_coords\t\t(короткий ключ -r)\n\t\tТребует два параметра: координаты левого верхнего угла области назначеия для функции копирования области изображения.\n\n");
                ...
                printf("  --frame_pattern\t\t(короткий ключ -P)\n\t\tОпция требует один параметр: узор (от 1 до 5).\n");
                ...
                printf("\t\tУзор рисуется по всему пириметру с шириной входа внутрь изображения и цветом, которые можно задать с помоью опций\n\t\t");
            ...
            case 'X':{
                ...
                // check_value(optarg);
                // // int a = atoi(optarg);
                // USER.hex_mode = atoi(optarg);
                // if (USER.hex_mode < 1 || USER.hex_mode > 2){
                //     fprintf(stderr, "Шестиугольник можно задать двумя способами: нужно задать режим рисования шестиуголника!\nПересмотрите инструкцию к программе.");
                //     exit(EXIT_FAILURE);
                // }
                break;}
    ...
    if (USER.source_file_name == NULL || USER.dest_file_name == NULL){
        fprintf(stderr, "Ошибка: вы не ввели опцию сохранения в файл (или путь) или сам исходный файл! Попробуйте еще раз.\n");
        exit(EXIT_FAILURE);
    }
    ...
    if ( !((USER.source_file_name[len1-1] == 'p' || USER.source_file_name[len1-1] == 'P') && (USER.source_file_name[len1-2] == 'm' || USER.source_file_name[len1-2] == 'M') && 
        ...
        {
            fprintf(stderr, "Ошибка: вы ввели имя или путь не к bmp файлу! Попробуйте снова.\n");
            exit(EXIT_FAILURE);
        }
