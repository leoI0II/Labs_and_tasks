...
std::vector<uint32_t> RabinKarpSearch(std::string& Text, std::string& Pattern, std::string& tochange){
...
void replaceW(std::vector<std::pair<std::string, std::string>>& tabl, std::string& text){
    for (int i = 0; i < tabl.size(); ++i){
        RabinKarpSearch(text, tabl[i].first, tabl[i].second);
    }
}
...
// #endif
    ...
    replaceW(patterns, text);

    // std::vector<uint32_t> result = RabinKarpSearch(text, pattern, change_pattern);
